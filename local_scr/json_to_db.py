...
def extract_promo_info(sec_name: Optional[str], product_name: Optional[str], description: Optional[str]) -> Tuple[str, int]:
    ...
    # 2. 匹配英文 "Buy X Get Y (Free)"
    pattern_en = r"Buy\s*([1-9])\s*Get\s*([1-9])"
    for text in texts:
                    ...
                    for sec in sections:
                        ...
                        if isinstance(items, list):
                            for item in items:
                                ...
                                generic_sections = {"精選商品", "熱門商品", "人氣精選", "熱門", "買 1 送 1", "買1送1", "促銷商品", "超值優惠"}
                                if pid in store_products:
                                    old_p = store_products[pid]
                                    # 若新舊任一處具有促銷活動標記，保留促銷活動與數量
                                    if promo_type != "無" and old_p["promo_type"] == "無":
                                        old_p["promo_type"] = promo_type
                                        old_p["quantity"] = qty

                                    # 若舊分類為通用推薦區塊，而新分類為具體類別，則更新為具體類別與價格
