...
- 前端「大特價」頁籤目前仍優先讀取 `web/data/discounts.json`。這份靜態資料的產生器採另一套口徑：與最近歷史批次的實質單價比較，至少降 20% 且省 20 元；UI 預設再篩到至少降 30% 且省 20 元。
- 新店家與新商品也有「正規化全歷史 first_seen」及「靜態前一批比較」兩種口徑。使用哪一個，取決於功能與資料來源；細節見規則文件。
