...
外送平台上常出現「買1送1」、「買2送1」、「第2件半價」、「加1元多1件」等促銷活動，若僅比對表面標價會失真。系統引進實質單件單價（`calculate_effective_price`）機制：

1. **買 $B$ 送 $F$（例如買 2 送 1，消費者付 2 件價格獲得 3 件）**：
   $$\text{實質單價} = \frac{\text{單品標價 } P \times B}{B + F}$$
   *範例：美式咖啡一杯 \$90，活動「買 2 送 1」（$B=2, F=1$），實質單價 $= \frac{90 \times 2}{3} = \$60$（相當於 67 折，降 33.3%）。*
2. **第 2 件半價**：
   $$\text{實質單價} = \frac{P \times (1 + 0.5)}{2} = 0.75 P$$
3. **加 1 元多 1 件**：
   $$\text{實質單價} = \frac{P + 1.0}{2}$$
4. **常規多入組合商品（如一盒 6 入）**：
   $$\text{實質單價} = \frac{\text{組合總價 } P}{\text{內含件數 } \text{quantity}}$$

---

### 5.2 大特價 (Big Discount) 判定標準與防呆過濾
以該商品 UUID (`product_id`) 之「今日批次實質單價」與「前次營業中快照實質單價」進行精準比對：
- **營業狀態雙向限定**：歷史基準價與當前價均強制限定在營業中快照（`is_open = 1 且 price >= 1`），徹底排除非營業時段（打烊）之牌價干擾。
- **無變動排除**：若前後快照實質單價相同（例如幾天前也是買 2 送 1，今天也是買 2 送 1，實質單價皆為 \$60），變動率為 0%，不列入今日降價。
- **防呆過濾 (Anti-Typo Filter)**：外送平台偶有店家漏打 0（例如原價 \$286 打成 \$28、\$668 打成 \$70 等導致降幅達 90% 之手誤標價）。系統自動過濾**無任何促銷標籤且降幅 > 80%** 之異常值，防止虛假情報干擾消費者。

設 $P_{prev}$ 為前次營業中快照之實質單價，$P_{curr}$ 為本次營業中之實質單價：

1. **降價幅度 (Discount Percentage)**：
   $$\text{discount\_pct} = \frac{P_{prev} - P_{curr}}{P_{prev}} \times 100\% \ge 20.0\% \quad (\text{推播警報門檻為 } 30.0\%)$$
2. **現省金額 (Savings Amount)**：
   $$\text{savings\_amount} = P_{prev} - P_{curr} \ge \$20.0\text{ 元}$$
