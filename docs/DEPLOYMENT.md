...
3. 以 `publish=true` 重跑；GitHub runner 使用 `TURSO_PLATFORM_TOKEN` 建立隔離的 `ubereats-rebuilt-v10`、產生暫時的 database token，並直接發布。禁止由使用者本機建立或寫入 production Turso。
4. 遠端數量與查詢驗證通過後，才把 production secrets 切到新庫並設定 repository variable `TURSO_ACTIVE_SCHEMA_VERSION=v10`。未設定此 gate 時，每日爬蟲只更新重建 cache，不會寫入任何 Turso。
