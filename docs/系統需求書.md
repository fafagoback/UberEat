...
- **程式檔案**：`src/taiwan_menu_worker.py` (基於 `location_batch_scraper.py` 4.0 核心)
- **執行環境**：15 台平行 Ubuntu Runner (`strategy.matrix: chunk_id: 0..14`)
- **行為**：
...
- **程式檔案**：`src/json_to_cf_d1.py`, `src/json_to_db.py`, `src/alert_engine.py`
- **行為**：
...
2. `src/taiwan_menu_worker.py` (新建：專門負責執行 Stage 4 菜單抓取，並在抓完後呼叫 `upload_to_hf.py` 獨立 Commit 上傳)
3. `src/taiwan_store_reducer.py` (修改：擴充支援輸出 `menu_tasks/chunk_0~14.json`，並直接推送 `taiwan_stores_dataset` 至 HF)
4. `src/upload_to_hf.py` (修改：強化指數退避與隨機擾動重試機制，防止 15 台並行 Commit 衝突)
5. `src/json_to_cf_d1.py` (修改：支援全台 100 萬筆規模之 SQL 自動分塊寫入引擎)
